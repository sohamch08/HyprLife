...
PowerStatus=$(cat /sys/class/power_supply/BAT0/status) || exit 1
state_file="${XDG_RUNTIME_DIR:-/run/user/$(id -u)}/waybar-battery-status"
previous=$(cat "$state_file" 2>/dev/null) || previous=

[ "$PowerStatus" = "$previous" ] && exit 0

case "$status" in
Charging | Discharging)
  notify-send -u normal 'Power Switch' "$status" -h string:x-canonical-private-synchronous:powerstatus || exit 1
  ;;
esac
